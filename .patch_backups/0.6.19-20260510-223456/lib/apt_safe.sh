...
# - does not treat apt-get update with "Temporary failure resolving" as OK
...
apt_update_safe() {
  ...
  for attempt in $(seq 1 "$APT_UPDATE_MAX_ATTEMPTS"); do
    ...
    safe_log OK "apt-get update (attempt $attempt/$APT_UPDATE_MAX_ATTEMPTS)"
    ...
    if grep -Eqi 'Temporary failure resolving|Could not resolve|Failed to fetch|Some index files failed|Не удалось получить|Временная ошибка при разрешении|Некоторые индексные файлы скачать не удалось' "$out"; then
      safe_log WARN "apt-get update had DNS/fetch warnings, retrying"
      sleep 3
      write_apt_resolv_conf
      continue
    fi
    ...
    if [ "$rc" -eq 0 ]; then
      safe_log OK "apt-get update completed cleanly"
      return 0
    fi

    safe_log WARN "apt-get update failed with code $rc"
    sleep 3
  done

  safe_log FAIL "apt-get update failed after $APT_UPDATE_MAX_ATTEMPTS attempts"
  return 1
}
