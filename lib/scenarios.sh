...
scenario_hq_rtr() {
  ...
  prompt_default HQ_CLI_NET_IP_CIDR "HQ client network IP/VLAN, optional" "192.168.200.1/27"
  ...
  STATIC_ROUTES=""
  if [ -n "$HQ_CLI_NET_IP_CIDR" ]; then
    IPV4_CONFIGS="$IPV4_CONFIGS $LAN_IFACE.200:$HQ_CLI_NET_IP_CIDR"
    INTERFACES="$INTERFACES $LAN_IFACE.200"
  fi
