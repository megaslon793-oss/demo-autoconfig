...
patch_file() {
  ...
  perl -0pi -e 's/Router SSH port \[\s*22\s*\]/Router SSH port [22]/g' "$f"

  # 2. Any line containing Router SSH port and 22.
  # Examples:
  # prompt "Router SSH port" "22"
  # ask "Router SSH port [22]:"
  # read_value "Router SSH port" SSH_ROUTER_PORT "22"
...
grep -R "Router SSH port.*22\\|SSH_ROUTER_PORT.*2026" -n . \
  --exclude-dir=.git \
